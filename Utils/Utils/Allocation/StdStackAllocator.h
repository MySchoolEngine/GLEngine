...
public:
  ...
  C_StdStackAllocator() : m_head(m_memory), m_Allocated(0) {}

  template <class U>
  C_StdStackAllocator(
      const C_StdStackAllocator<U, size, alignment> &other) noexcept
      : m_head(m_memory) {}
  ...
  void deallocate(pointer p, size_type n) {
    if (reinterpret_cast<decltype(m_head)>(p) +
            AllocUtils::roundToAligned(n * sizeof(T)) ==
        m_head) {
...
private:
  ...
  using AllocUtils =
      C_AllocatorUtils<C_StdStackAllocator<T, size, alignment>, alignment>;
};

template <class T, std::size_t size, std::size_t alignment>
bool operator==(const C_StdStackAllocator<T, size, alignment> &lhs,
                const C_StdStackAllocator<T, size, alignment> &rhs) {
