...
public:	
	...
	template <class U>
	struct rebind { using other = C_StdStackAllocator<U, size, alignment>; };
	...
	bool owns(pointer p)
	{
		return p >= m_memory && p < m_memory + size;
	}
...
bool operator==(const C_StdStackAllocator<T, size, alignment>& lhs, const C_StdStackAllocator<T, size, alignment>& rhs) {
