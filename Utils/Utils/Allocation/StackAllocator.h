...
template<std::size_t size, std::size_t alignment = 4>
class C_StackAllocator
{
public:
	...
	template<class T>
	void deallocateArray(T* p, size_type n)
	{
	...
	bool owns(T* p)
	{
		return static_cast<decltype(m_head)>(p) >= m_memory && static_cast<decltype(m_head)>(p) < m_memory + size;
	}
