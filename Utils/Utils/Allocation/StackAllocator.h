...
public:
  ...
  C_StackAllocator() : m_head(m_memory) {}
  ...
  template <class T> void deallocate(T *p) {
    if (reinterpret_cast<decltype(m_head)>(p) +
            AllocUtils::roundToAligned(sizeof(T)) ==
        m_head) {
  ...
  template <class T> void deallocateArray(T *p, size_type n) {
    if (reinterpret_cast<decltype(m_head)>(p) +
            AllocUtils::roundToAligned(sizeof(T) * n) ==
        m_head) {
      m_head = reinterpret_cast<decltype(m_head)>(p);
    }
  }
  ...
  template <class T> bool owns(T *p) {
    return static_cast<decltype(m_head)>(p) >= m_memory &&
           static_cast<decltype(m_head)>(p) < m_memory + size;
  }
