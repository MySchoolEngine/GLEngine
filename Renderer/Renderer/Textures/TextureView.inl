...
template <class T> void C_TextureView::Set(const glm::uvec2& coord, const T val, E_TextureChannel element)
{
	...
	m_Storage->Set(val, GetAddress(coord) + m_Storage->GetChannelOffset(element));
}

//=================================================================================
template <class T, typename /*= std::enable_if_t<glm::type<T>::is_vec>*/> void C_TextureView::Set(const glm::uvec2& coord, T&& val)
{
	// swizzle on views side
	if constexpr (std::is_same_v<std::remove_cv_t<T>, glm::vec4>)
	{
		m_Storage->SetPixel(val, GetPixelAddress(coord));
	}
	else if constexpr (std::is_same_v<std::remove_cv_t<T>, glm::vec3>)
	{
		m_Storage->SetPixel(glm::vec4(val, 0.f), GetPixelAddress(coord));
	}
	else
	{
		for (std::uint8_t i = 0; i < std::min(static_cast<std::uint8_t>(glm::type<T>::components), m_Storage->GetNumElements()); ++i)
		{
			Set(coord, val[i], static_cast<E_TextureChannel>(i));
		}
	}
}
...
template <class T> void C_OctahedralTextureView::Set(const glm::vec3& direction, const T val, E_TextureChannel element)
{
	...
	const auto realCoord = glm::uvec2{static_cast<float>(m_Size) * (coord / 2.0f + 0.5f)};
