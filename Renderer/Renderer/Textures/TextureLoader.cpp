...
//=================================================================================
bool TextureLoader::loadTexture(const std::filesystem::path &path,
                                MeshData::Texture &t) {
  if (!_isILinitialized) {
    ilInit();
  ...
  if (Error != IL_NO_ERROR) {
    CORE_LOG(E_Level::Error, E_Context::Render,
             "DevIL: Failed to load image {}, error: {}", path, Error);
    t.height = t.width = 1;
    t.m_name = "Error";
    t.data = std::shared_ptr<unsigned char>(
        new unsigned char[4 * t.width * t.height]);
    t.data.get()[0] = 255;
    t.data.get()[1] = 0;
    t.data.get()[2] = 0;
    t.data.get()[3] = 0;
  ...
  t.data =
      std::shared_ptr<unsigned char>(new unsigned char[4 * t.width * t.height]);
...
TextureLoader::loadTexture(const std::filesystem::path &path) {
  if (!_isILinitialized) {
    ilInit();
    _isILinitialized = true;
  }
  ...
  if (ilGetInteger(IL_IMAGE_BPC) == 1) {
    ...
    CORE_LOG(E_Level::Error, E_Context::Render,
             "Image have unsupported bytes per channel {}",
  ...
  case IL_RGB: {
    textureBuffer->SetChannels({E_TextureChannel::Red, E_TextureChannel::Green,
                                E_TextureChannel::Blue,
                                E_TextureChannel::None});
    break;
  }
  case IL_RGBA: {
    textureBuffer->SetChannels({E_TextureChannel::Red, E_TextureChannel::Green,
                                E_TextureChannel::Blue,
                                E_TextureChannel::Alpha});
    break;
  }
  case IL_BGR: {
    textureBuffer->SetChannels({E_TextureChannel::Blue, E_TextureChannel::Green,
                                E_TextureChannel::Red, E_TextureChannel::None});
    break;
  }
  case IL_BGRA: {
    textureBuffer->SetChannels({E_TextureChannel::Blue, E_TextureChannel::Green,
                                E_TextureChannel::Red,
                                E_TextureChannel::Alpha});
    break;
  }
  default: {
    CORE_LOG(E_Level::Error, E_Context::Render,
             "Image have unsupported format {}", ilGetInteger(IL_IMAGE_FORMAT));
    break;
  }
  }
