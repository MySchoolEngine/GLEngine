...
public:
  [[nodiscard]] static Mesh CreatePlane(int subdivisions = 0) {
    ...
    Mesh mesh;
    mesh.vertices.reserve(6 * static_cast<std::size_t>(std::pow(
                                  4.0, static_cast<double>(subdivisions))));
    ...
    std::fill(mesh.bitangent.begin(), mesh.bitangent.end(),
              glm::vec3(1.f, 0.f, 0.f));
    std::fill(mesh.tangent.begin(), mesh.tangent.end(),
