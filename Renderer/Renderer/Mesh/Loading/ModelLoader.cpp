...
//=================================================================================
ModelLoader::ModelLoader() : _importer(std::make_unique<Assimp::Importer>()) {}

//=================================================================================
ModelLoader::~ModelLoader() = default;

//=================================================================================
bool ModelLoader::addModelFromFileToScene(
    const std::filesystem::path &path,
    std::shared_ptr<Renderer::MeshData::Scene> scene,
    std::vector<std::string> &textureRegister, glm::mat4 sceneTransform) {
...
void ModelLoader::Reset() {
  _numMaterialsPreviouslyLoaded = _numMeshesPreviouslyLoaded =
      _numTexturesPreviouslyLoaded = 0;
}

//=================================================================================
void ModelLoader::_loadMaterialsFromAiscene(
  ...
  for (unsigned int i = 0; i < loadedScene->mNumMaterials; ++i) {
    ...
    const std::string texName =
        _getMaterialDiffuseTextureName(loadedScene->mMaterials[i]);
    m.textureIndex = _getTextureIndexAndAddToRegister(texName, textureRegister);

    const auto dispTexName =
        _getMaterialNormalTextureName(loadedScene->mMaterials[i]);
    m.noramlTextureIndex =
        _getTextureIndexAndAddToRegister(dispTexName, textureRegister);
...
const aiScene *ModelLoader::_tryOpenFile(const std::filesystem::path &path) {
  const aiScene *s = _importer->ReadFile(
  ...
  if (s == nullptr) {
    CORE_LOG(E_Level::Error, E_Context::Render, "Failed to import {}",
             path.generic_string());
    return nullptr;
  }
...
//=================================================================================
void ModelLoader::_getMaterialColorAttributes(
    const aiMaterial *const material, Renderer::MeshData::Material &mat) {
...
//=================================================================================
glm::vec4
ModelLoader::_getMaterialColorComponent(const aiMaterial *const material,
...
//=================================================================================
float ModelLoader::_getMaterialFloatComponent(const aiMaterial *material,
                                              const char *key, unsigned type,
                                              unsigned index) {
  if (material) {
    float f;
...
//=================================================================================
std::string
ModelLoader::_getMaterialDiffuseTextureName(const aiMaterial *material) {
...
//=================================================================================
std::string
ModelLoader::_getMaterialNormalTextureName(const aiMaterial *material) {
...
//=================================================================================
int ModelLoader::_getTextureIndexAndAddToRegister(
    const std::string &name, std::vector<std::string> &textureNames) {
  if (name.empty())
    return -1;
...
//=================================================================================
void ModelLoader::_loadLightsFromAiScene(
    const aiScene *loadedScene,
    std::shared_ptr<Renderer::MeshData::Scene> scene) {
...
//=================================================================================
void ModelLoader::_loadMeshesFromAiScene(
    const aiScene *loadedScene,
    std::shared_ptr<Renderer::MeshData::Scene> scene,
    const glm::mat4 &sceneTransform) {
  const aiNode *currentNode = loadedScene->mRootNode;
  glm::mat4 currentTransform =
      sceneTransform * _aiMatrixToGlm(loadedScene->mRootNode->mTransformation);

  while (currentNode != nullptr) {
    _pushNodeChildrenOnStacks(currentNode, currentTransform);

    _loadNodeMeshes(currentNode, currentTransform, loadedScene->mMeshes,
                    scene->meshes);
...
//=================================================================================
void ModelLoader::_pushNodeChildrenOnStacks(
    const aiNode *node, const glm::mat4 &nodeWorldTransform) {
  const unsigned int numChildren = node->mNumChildren;

  for (unsigned int i = 0; i < numChildren; ++i) {
    _nodeStack.push(node->mChildren[i]);
    _transformStack.push(nodeWorldTransform *
                         _aiMatrixToGlm(node->mTransformation));
  }
}

//=================================================================================
void ModelLoader::_getNextNodeAndTransform(const aiNode *&node,
                                           glm::mat4 &transform) {
  if (_nodeStack.empty()) {
    node = nullptr;
    return;
...
//=================================================================================
void ModelLoader::_loadNodeMeshes(
    const aiNode *node, const glm::mat4 &nodeTransform, aiMesh **const aiMeshes,
    std::vector<Renderer::MeshData::Mesh> &meshes) {
  _allocateNewMeshes(node->mNumMeshes, meshes);

  for (unsigned int i = 0; i < node->mNumMeshes; ++i) {
    const unsigned int meshIndex = node->mMeshes[i];
    _loadSingleMeshFromAimesh(aiMeshes[meshIndex],
                              meshes[_numMeshesPreviouslyLoaded + i]);

    meshes[_numMeshesPreviouslyLoaded + i].modelMatrix = nodeTransform;
    meshes[_numMeshesPreviouslyLoaded + i].m_name =
...
//=================================================================================
void ModelLoader::_loadSingleMeshFromAimesh(const aiMesh *aiMesh,
...
//=================================================================================
void ModelLoader::_allocateNewMeshes(
    const unsigned int numNewMeshes,
    std::vector<Renderer::MeshData::Mesh> &meshes) {
  if (numNewMeshes)
...
//=================================================================================
void ModelLoader::_allocateMesh(Renderer::MeshData::Mesh &mesh,
                                const unsigned int numFaces) {
  mesh.vertices.resize(static_cast<std::size_t>(numFaces) *
                       VERTICES_PER_TRIANGLE);
  mesh.normals.resize(static_cast<std::size_t>(numFaces) *
                      VERTICES_PER_TRIANGLE);
  mesh.texcoords.resize(static_cast<std::size_t>(numFaces) *
                        VERTICES_PER_TRIANGLE);
  mesh.tangent.resize(static_cast<std::size_t>(numFaces) *
                      VERTICES_PER_TRIANGLE);
  mesh.bitangent.resize(static_cast<std::size_t>(numFaces) *
                        VERTICES_PER_TRIANGLE);
}

//=================================================================================
void ModelLoader::_assignMeshMaterial(Renderer::MeshData::Mesh &mesh,
                                      const aiMesh *aimesh) {
  mesh.materialIndex = aimesh->mMaterialIndex + _numMaterialsPreviouslyLoaded;
}

//=================================================================================
void ModelLoader::_getFacePosNormalTcoords(
    const aiFace *face, const aiMesh *mesh, glm::vec4 *pos, glm::vec3 *normal,
    glm::vec2 *tcoords, glm::vec3 *ttangents, glm::vec3 *tbitangents) {
  GLE_ASSERT(face->mNumIndices == VERTICES_PER_TRIANGLE,
             "Triangle should have {} vertices", VERTICES_PER_TRIANGLE);
