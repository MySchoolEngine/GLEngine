...
bool C_ColladaLoader::addModelFromDAEFileToScene(
    const char *filepath, const char *filename, MeshData::Mesh &oMesh,
    std::string &textureName, C_Skeleton &skeleton,
    C_SkeletalAnimation &animation, MeshData::AnimationData &animData,
    glm::mat4 &transform) {
  std::string name;
  ...
  if (auto up_Axis = colladaNode.child("asset").child("up_axis")) {
    ...
    if (upAxe == "Z_UP") {
      noramlizingMatrix =
          glm::rotate(-glm::half_pi<float>(), glm::vec3(1.f, .0f, .0f));
    } else if (upAxe == "Y_UP") {
      // do nothing
    } else {
      CORE_LOG(E_Level::Error, E_Context::Core, "unknown <up_axis> value: {}",
               upAxe);
    }
  }

  if (auto imageLibrary = colladaNode.child("library_images")
                              .child("image")
                              .child("init_from")) {
      ...
      if (auto xmlMesh = geom.child("mesh")) {
        for (auto xmlSource : xmlMesh.children("source")) {
          ...
          if (id.find("positions") != id.npos) {
            ...
            while (!floats.EndOfArray()) {
              vertices.emplace_back(noramlizingMatrix *
                                    glm::vec4(floats.Get<glm::vec3>(), 1.f));
            }
        ...
        if (auto polylist = xmlMesh.child("polylist")) {
          ...
          if (const auto countAttribute = polylist.attribute("count")) {
            ...
            CORE_LOG(E_Level::Warning, E_Context::Core,
                     "<triangles> element misses required attribute count.");
          }
  ...
  if (auto controllerLibrary = colladaNode.child("library_controllers")) {
    if (auto skinXML = controllerLibrary.child("controller").child("skin")) {
      if (const auto BSMatrix = skinXML.child("bind_shape_matrix")) {
        ...
        transform =
            glm::transpose(glm::mat4(f1, f2, f3, f4, f5, f6, f7, f8, f9, f10,
                                     f11, f12, f13, f14, f15, f16));
      }
      LoadJointsInvMatrices(joints, skinXML, noramlizingMatrix);
    }
  ...
  if (auto visualScene =
          colladaNode.child("library_visual_scenes").child("visual_scene")) {
    for (auto xmlNode : visualScene.children("node")) {
      ...
      if (id == "Armature") {
        ...
        auto modelMatrix =
            Utils::Parsing::C_MatrixParser::ParseTransformation(xmlNode);
        if (std::distance(xmlNode.children("node").begin(),
                          xmlNode.children("node").end()) != 1) {
          CORE_LOG(E_Level::Warning, E_Context::Render,
                   "Skeleton definition should contain only 1 root joint");
        }
        ...
        if (rootJoint == joints.end()) {
          CORE_LOG(E_Level::Error, E_Context::Render,
                   "Bone '{}' referenced in file '{}' doesn't exiests.",
                   boneName, name);
          return false;
        }
        ...
        if (!ParseChildrenJoints(*skeleton.m_Root.get(), rootNode, joints)) {
          CORE_LOG(E_Level::Error, E_Context::Render,
                   "Errors in joint hierarchy definition in file: {}", name);
        }
      }
    }
  }
  ...
  if (auto animationLibrary = colladaNode.child("library_animations")) {
    ...
    for (const auto &animationXML : animationLibrary.children("animation")) {
      ...
      std::string boneIdName(id.substr(
          underscoreIndex, id.find("_pose_matrix") - underscoreIndex));
      const auto boneId = GetBoneId(boneIdName);
      if (boneId < 0) {
        CORE_LOG(E_Level::Warning, E_Context::Render,
                 "Referenced bone '{}' but not found", boneIdName);
        continue;
      }
      animation.SetBoneTimeline(
          boneId, LoadBoneTimeline(animationXML, noramlizingMatrix));
    }
  }
...
//=================================================================================
void C_ColladaLoader::LoadJointsInvMatrices(
    std::map<std::string, S_Joint> &joints, const pugi::xml_node &skinXML,
    const glm::mat4 &normalizinMatrix) const {
  for (auto xmlSource : skinXML.children("source")) {
    const std::string_view id = xmlSource.attribute("id").as_string();
    ...
    if (const auto floatArray = xmlSource.child("float_array")) {
      ...
      for (const auto &name : m_JointNames) {
        const auto bindMatrix =
            glm::transpose(normalizinMatrix * floats.Get<glm::mat4>());
...
//=================================================================================
bool C_ColladaLoader::ParseChildrenJoints(
    S_Joint &parent, const pugi::xml_node &xmlParent,
    const std::map<std::string, S_Joint> &joints) {
  for (const auto &node : xmlParent.children("node")) {
    ...
    if (joint == joints.end()) {
      CORE_LOG(E_Level::Error, E_Context::Render,
               "Bone '{}' referenced doesn't exiests.", boneName);
      continue;
      // return false;
    }
...
//=================================================================================
void C_ColladaLoader::Reset() { m_JointNames.clear(); }

//=================================================================================
C_BoneTimeline
C_ColladaLoader::LoadBoneTimeline(const pugi::xml_node &node,
                                  const glm::mat4 &normalizinMatrix) const {
  ...
  while (floatArrays[0].EndOfArray() == false) {
    timeline.AddBoneKeyFrame(
        index, S_BoneKeyframe(glm::transpose(floatArrays[1].Get<glm::mat4>()),
                              floatArrays[0].Get<float>()));
    ++index;
  }
...
//=================================================================================
void C_ColladaLoader::LoadAnimData(const pugi::xml_node &skinXML,
                                   std::vector<glm::ivec3> &jointIndices,
  ...
  if (auto vertex_weights = skinXML.child("vertex_weights")) {
    ...
    while (vcount.EndOfArray() == false) {
      ...
      if (jointsNum > 3) {
        CORE_LOG(E_Level::Warning, E_Context::Render,
                 "Only 3 joints per vertex are supported");
      }
      for (int i = 0; i < 3; ++i) {
        if (i >= jointsNum) {
          jointIndicesVec[i] = 0;
