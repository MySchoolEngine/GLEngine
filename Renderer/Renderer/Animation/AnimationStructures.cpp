...
//=================================================================================
S_Timestamp::S_Timestamp() : m_Timestamp(0.f) {}

//=================================================================================
float S_Timestamp::Clamp(float f) { return std::clamp(f, 0.0f, 1.0f); }
...
S_BoneKeyframe::S_BoneKeyframe(const glm::mat4 &matrix, S_Timestamp timestamp)
    : m_Transform(matrix[3][0], matrix[3][1], matrix[3][2]),
      m_Rotation(glm::quat_cast(matrix)), m_Timestamp(timestamp) {}

//=================================================================================
S_BoneKeyframe::S_BoneKeyframe(const glm::vec3 &transformation,
                               const glm::quat &rotation, S_Timestamp timestamp)
    : m_Transform(transformation), m_Rotation(rotation),
...
//=================================================================================
void C_BoneTimeline::AddBoneKeyFrame(std::size_t index,
                                     S_BoneKeyframe &&keyframe) {
  GLE_ASSERT(keyframe.m_Timestamp >= m_Timeline.back().m_Timestamp,
             "Adding keyframes out of order not suppoerted");
...
S_BoneKeyframe C_BoneTimeline::Sample(S_Timestamp timestamp) const {
  ...
  const auto &nextKeyframeIt =
      std::find_if(m_Timeline.begin(), m_Timeline.end(),
                   [timestamp](const S_BoneKeyframe &it) {
                     return it.m_Timestamp > timestamp;
                   });
  ...
  const auto intervalLen =
      nextKeyframe.m_Timestamp - previousKeyframe.m_Timestamp;
  const auto timeInInterval = timestamp - previousKeyframe.m_Timestamp;
  const auto percentageProgress =
      timeInInterval.GetValue() / intervalLen.GetValue();
  // return result
  return S_BoneKeyframe(glm::mix(previousKeyframe.m_Transform,
                                 nextKeyframe.m_Transform, percentageProgress),
                        glm::slerp(previousKeyframe.m_Rotation,
                                   nextKeyframe.m_Rotation, percentageProgress),
                        timestamp);
}
} // namespace GLEngine::Renderer::Animation