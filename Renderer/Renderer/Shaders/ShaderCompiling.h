...
public:
  ...
  bool compileShaderStage(T_StageHandle &stage,
...
protected:
  ...
  virtual bool compileShaderStageInternal(
      T_StageHandle &stage, const std::filesystem::path &filepath,
      const Renderer::E_ShaderStage shaderStage, std::string &content) = 0;
...
public:
  ...
  bool LoadAllStages(const std::filesystem::path &name,
                     std::vector<T_ShaderStage> &stages) {
    pugi::xml_document doc;

    if (!LoadDoc(doc, name)) {
      CORE_LOG(E_Level::Error, E_Context::Render,
               "Can't open config file for shader name: {}", name);
      return false;
    }
...
private:
  ...
  bool LoadShaderStage(const pugi::xml_node &node, T_ShaderStage &stagePair) {
    ...
    const auto filename =
        s_ShadersFolder / std::filesystem::path(node.first_child().value());
  ...
  //=================================================================================
  bool LoadDoc(pugi::xml_document &document,
...
const std::filesystem::path C_ShaderLoader<StageHandle>::s_ShadersFolder =
    "shaders/";
...
//=================================================================================
template <class StageHandle>
bool ShaderCompilerTrait<StageHandle>::compileShaderStage(
    T_StageHandle &stage, const std::filesystem::path &filepath,
    const Renderer::E_ShaderStage shaderStage) {
  std::string src;
  if (!_loadFile(filepath, src)) {
    CORE_LOG(E_Level::Error, E_Context::Render,
             "Failed to open shader file: {}", filepath.generic_string());
    return false;
  }
...
//=================================================================================
template <class StageHandle>
typename ShaderCompilerTrait<StageHandle>::T_Paths
ShaderCompilerTrait<StageHandle>::GetTouchedFiles() const {
  return m_TouchedFiles;
}

//=================================================================================
template <class StageHandle>
bool ShaderCompilerTrait<StageHandle>::_loadFile(
    const std::filesystem::path &file, std::string &content) {
  std::ifstream stream(file);

  if (stream.fail())
    return false;

  content = std::string(std::istream_iterator<char>(stream >> std::noskipws),
