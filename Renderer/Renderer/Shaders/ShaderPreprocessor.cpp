...
//=================================================================================
const std::regex C_ShaderPreprocessor::s_IncludeFileName =
    std::regex(R"(^(#include )\"([^\"]*)\"$)");
const std::regex C_ShaderPreprocessor::s_GenerateStruct =
    std::regex(R"(^(@struct )([^\"\n;\s]*))");
const std::regex C_ShaderPreprocessor::s_DefineRegEx =
    std::regex(R"(^(#define )([^\s]*)\s([^\s]+)$)");
const std::regex C_ShaderPreprocessor::s_IfDefinedRegEx =
    std::regex(R"(^(#if )(!?)(defined)\s([^\s]+)$)");
const std::regex C_ShaderPreprocessor::s_EndIfDefinedRegEx =
    std::regex(R"(^#endif([\s]*\/\/.*)?$)");

//=================================================================================
C_ShaderPreprocessor::C_ShaderPreprocessor(
    std::unique_ptr<I_CodeProvider> &&codeProvider)
...
//=================================================================================
void C_ShaderPreprocessor::Define(const std::string &symbol,
...
//=================================================================================
std::string
C_ShaderPreprocessor::PreprocessFile(const std::string &src,
...
//=================================================================================
void C_ShaderPreprocessor::IncludesFiles(
    std::string &content, const std::filesystem::path &folderPath) {
  std::smatch m;
  std::string result = "";
...
bool C_ShaderPreprocessor::_loadFile(const std::filesystem::path &file,
                                     std::string &content) {
...
void C_ShaderPreprocessor::ResolveIfStatements(std::string &content) {
  ...
  while (std::regex_search(content, m, s_IfDefinedRegEx)) {
    ...
    if (!std::regex_search(rest, endIfMatch, s_EndIfDefinedRegEx)) {
      CORE_LOG(E_Level::Error, E_Context::Render,
               "No matching #endif for given #if statement.");
