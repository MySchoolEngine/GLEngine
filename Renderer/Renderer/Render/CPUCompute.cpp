	...
	if (target.GetStorage()->GetNumElements() == 1)
	{
		for (unsigned int x = 0; x < view.GetDimensions().x; ++x)
		{
			for (unsigned int y = 0; y < view.GetDimensions().y; ++y) {
				const glm::vec3 val = view.Get<glm::vec3>(glm::ivec2{x, y});
				const auto		valLuminance = Colours::ToLuminance(val);
				int				bucket		 = valLuminance / bucketSize;
				target.Set(glm::ivec2{bucket, 0}, target.Get<float>(glm::ivec2{bucket, 0}, E_TextureChannel::Red) + 1.f, E_TextureChannel::Red);
			}
		}
	}


	CORE_LOG(E_Level::Info, E_Context::Render, "Histogram generation time {}s", float(computeHistogramTimer.getElapsedTimeFromLastQueryMilliseconds()) / 1000.f);
}
