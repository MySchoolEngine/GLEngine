...
//=================================================================================
C_RayRenderer::C_RayRenderer(const C_RayTraceScene &scene) : m_Scene(scene) {}

//=================================================================================
void C_RayRenderer::Render(I_CameraComponent &camera,
                           I_TextureViewStorage &storage) const {
  const auto low1 = 0.0;
  ...
  const auto GetRay = [&](int x, int y) {
    const auto clipSpace =
        ToClipSpace({static_cast<float>(x), static_cast<float>(y)});
  ...
  const C_Primitive<Physics::Primitives::S_Disc> areaLight(
      Physics::Primitives::S_Disc(lightNormal, glm::vec3(0, 1, 2), .25f));
  ...
  for (int y = 0; y < dim.y; ++y) {
    for (int x = 0; x < dim.x; ++x) {
      ...
      if (areaLight.Intersect(ray, intersect)) {
        textureView.Set({x, y},
                        glm::vec3{areLightPower, areLightPower, areLightPower});
        continue;
      }
      ...
      C_RayIntersection intersectLight;
      const Physics::Primitives::S_Ray lightRay{
          intersect.GetIntersectionPoint(),
          UniformSampleHemisphere({distrib(gen), distrib(gen)})};
      ...
      const auto cosTheta =
          frame.CosTheta(frame.ToLocal(glm::normalize(lightRay.direction)));
      const auto sample =
          intersect.GetMaterial()->diffuse * cosTheta * (pointLightIntensity);

      textureView.Set({x, y},
                      textureView.Get<glm::vec3>({x, y}) +
                          glm::vec3{sample.x, sample.y,
                                    sample.z}); // Map due to texture format
