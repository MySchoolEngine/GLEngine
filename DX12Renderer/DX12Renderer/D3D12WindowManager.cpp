...
//=================================================================================
C_D3D12WindowManager::C_D3D12WindowManager(
    Core::C_Application::EventCallbackFn eventCallback, HINSTANCE hInstance)
    : Core::I_WindowManager(eventCallback), m_hInstance(hInstance),
      m_UpdatingWindow(nullptr) {
  Init();
}

//=================================================================================
std::shared_ptr<Core::I_Window>
C_D3D12WindowManager::OpenNewWindow(const Core::S_WindowInfo &info) {
  if (info.GetDriver() != Core::E_Driver::DirectX12) {
    return nullptr;
  }
  (dynamic_cast<S_D3D12WindowInfo *>(&const_cast<Core::S_WindowInfo &>(info)))
...
//=================================================================================
std::shared_ptr<GLEngine::Core::I_Window>
C_D3D12WindowManager::GetWindow(GUID guid) const {
  auto it = std::find_if(
      m_Windows.begin(), m_Windows.end(),
      [&guid](const std::shared_ptr<GLEngine::Core::I_Window> &wnd) {
...
//=================================================================================
C_D3D12WindowManager *
ConstructD3D12Manager(Core::C_Application::EventCallbackFn eventCallback,
                      HINSTANCE hInstance) {
