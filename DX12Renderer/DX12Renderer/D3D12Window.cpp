...
C_D3D12Window::C_D3D12Window(const Core::S_WindowInfo &wndInfo)
    : m_Window(nullptr), m_HInstance(nullptr), m_WantClose(false) {
  Init(wndInfo);
}

//=================================================================================
C_D3D12Window::~C_D3D12Window() { Destroy(); }
...
void C_D3D12Window::Init(const Core::S_WindowInfo &wndInfo) {
  const S_D3D12WindowInfo *d12wndInfo =
      (dynamic_cast<const S_D3D12WindowInfo *>(&wndInfo));
  ...
  RECT windowRect = {0, 0, static_cast<LONG>(wndInfo.m_width),
                     static_cast<LONG>(wndInfo.m_height)};
  AdjustWindowRect(&windowRect, WS_OVERLAPPEDWINDOW, FALSE);

  m_Window = CreateWindow(
      windowClass.lpszClassName,
      std::wstring(wndInfo.m_WindowClass.begin(), wndInfo.m_WindowClass.end())
          .c_str(),
...
//=================================================================================
bool C_D3D12Window::WantClose() const { return m_WantClose; }
...
//=================================================================================
void C_D3D12Window::Destroy() { DestroyWindow(m_Window); }

//=================================================================================
LRESULT CALLBACK C_D3D12Window::WindowProc(HWND hWnd, UINT message,
                                           WPARAM wParam, LPARAM lParam) {
  auto *window =
      reinterpret_cast<C_D3D12Window *>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
  switch (message) {
  case WM_CREATE: {
